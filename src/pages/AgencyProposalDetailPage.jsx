...
const AgencyProposalDetail = () => {
  ...
  const getStatusBadge = (status) => {
    ...
    return statusMap[status] || { text: '상태 없음', className: 'bg-gray-100 text-gray-800' };
  };
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-2 w-[800px] min-h-[400px] overflow-hidden">
        <div className="space-y-4">
          ...
          <div className="bg-gray-50 p-4 rounded-lg">
            <h3 className="text-lg font-semibold">{daySchedule.dayNumber}일차</h3>
          </div>
          <div className="ml-4"> {/* 일정 목록 오른쪽 이동 */}
  ...
  if (!proposal) return <p className="text-center">제안서 정보를 찾을 수 없습니다.</p>;
              ...
              <div className="h-[500px]">
                {activeTab === '상세 내용' && (
                  <div className="space-y-6">
                    <h2 className="text-xl font-bold">여행 상세 내용</h2><br></br>
                    <div className="bg-gray-50 p-4 rounded-lg">
                      <h3 className="font-bold mb-2">여행정보</h3>
                      <span>{proposal.productInformation}</span>
                    </div>
                    <div className="bg-gray-50 p-4 rounded-lg">
                      <h3 className="font-bold mb-2">여행 일정(datetime)</h3>
                      <p>출발: {formatDateTime(proposal.startDateBoardingTime)}</p>
                      <p>도착: {formatDateTime(proposal.startDateArrivalTime)}</p>
                    </div>
                    <div className="bg-gray-50 p-4 rounded-lg">
                      <h3 className="font-bold mb-2">항공사(airLine)</h3>
                      <p>{proposal.airLine}</p>
                    </div>
                    <div className="bg-gray-50 p-6 rounded-lg">
                      <h3 className="font-bold mb-2">탑승 및 도착 시간(datetime)</h3><br></br>
                      <div className="flex gap-24">
                        <div className="w-[280px]">
                          <p>출국 공항 : {proposal.departureAirport}</p><br></br>
                          <p>탑승 시간 : </p>
                          <p>{formatDateTime(proposal.startDateBoardingTime)}</p><br></br>
                          <p>도착 시간 :</p>
                    ...
                    <div className="bg-gray-50 p-4 rounded-lg">
                      ...
                      <div className="flex gap-4">
                        <label className="flex items-center">
                          <input type="radio" checked={proposal.guideIncluded} readOnly className="mr-2" />
                          포함
                        </label>
                        <label className="flex items-center">
                          <input type="radio" checked={!proposal.guideIncluded} readOnly className="mr-2" />
                    ...
                    <div className="bg-gray-50 p-4 rounded-lg">
                      <h3 className="font-bold mb-2">여행자 보험</h3>
                      <div className="flex gap-4">
                        <label className="flex items-center">
                          <input type="radio" checked={proposal.insuranceIncluded} readOnly className="mr-2" />
                          포함
                        </label>
                        <label className="flex items-center">
                          <input type="radio" checked={!proposal.insuranceIncluded} readOnly className="mr-2" />
                    ...
                    <div className="bg-gray-50 p-4 rounded-lg">
                      <h3 className="font-bold mb-2">취소/환불 정책</h3>
                      <p>{proposal.refundPolicy}</p>
                    </div>
                  </div>
                )}
                {activeTab.includes('일차') && (
                  <div className="space-y-6">{renderDaySchedule(activeTab.replace('일차', ''))}</div>
                )}
              </div>
          </div>
