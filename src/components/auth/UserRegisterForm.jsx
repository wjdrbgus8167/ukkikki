...
const UserRegisterForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await publicRequest.post('/api/v1/auth/users/register', formData);
      alert('회원가입 성공!');
    } catch (error) {
  ...
  return (
    <div className="w-full max-w-md mx-auto bg-white p-6 rounded-lg">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button
          type="submit"
          className="w-full bg-brown text-white py-2 rounded hover:bg-yellow hover:text-brown transition"
        >
          회원가입
        </button>
      </form>
