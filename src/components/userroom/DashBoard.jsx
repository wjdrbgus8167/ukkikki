  ...
  useEffect(() => {
    ...
    const fetchImage = async () => {
      const response = await fetch(
        `https://api.unsplash.com/photos/random?query=Japan&client_id=${apiKey}`,
      );
  ...
  return (
    <div className="flex flex-col md:flex-row justify-between items-center bg-gray-100 p-8">
      ...
      <div className="md:w-1/3 p-4">
        ...
        <p className="text-gray-700">{selectedCard.start_date}</p>
        <p className="text-gray-700">{selectedCard.end_date}</p>
        <p className="text-gray-700">{selectedCard.country}</p>
      </div>
